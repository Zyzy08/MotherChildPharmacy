...
document.getElementById("export_PDF").addEventListener("click", function () {
    ...
    logo.src = '../resources/img/logo.png'; // Replace with the path to your logo
    logo.style.width = '50px'; // Adjust size as needed
    logo.style.marginRight = '10px'; // Space between logo and title
    ...
    html2pdf().set({
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            // Footer text
            const footerText = `Date Generated: ${formattedDate}    |    Page ${i} of ${totalPages}`;
            const pageWidth = pdf.internal.pageSize.getWidth();
            const textWidth = pdf.getTextWidth(footerText);
            const xPosition = (pageWidth - textWidth) / 2; // Centering calculation
...
document.getElementById("export_PDF_inv").addEventListener("click", function () {
    ...
    html2pdf().set({
        ...
        pagebreak: {
            mode: ['avoid-all', 'css'] // Avoid splitting tables and respect CSS rules
        },
        filename: `Inventory_Report_${today.toISOString().split('T')[0]}.pdf`
    }).from(container).toPdf().get('pdf').then(function (pdf) {
        // Get total number of pages
        const totalPages = pdf.internal.getNumberOfPages();
