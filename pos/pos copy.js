...
document.getElementById('print-button').addEventListener('click', function() {
    ...
    .then(data => {
        if (data.success) {
            ...
            // After successful save, proceed with printing
            printReceipt();
        } else {
...
function printReceipt() {
    ...
    printWindow.onload = function() {
        try {
            const printer = printWindow.navigator.printer;
            if (printer) {
                printer.name = 'XP-58';
            }
        } catch (e) {
            console.error('Unable to set printer:', e);
        }
...
function generateReceiptContent() {
    ...
    const charWidth = 2.5; // Approximate width of a character in mm
    ...
    function formatLine(left, right) {
        ...
        const middlePadding = Math.max(0, maxChars - left.length - right.length - leftPadding - rightPadding);
        return left + ' '.repeat(leftPadding) + ' '.repeat(middlePadding) + ' '.repeat(rightPadding) + right;
    }
    ...
    let content = `
        ...
        <body>
            <pre>
${centerText('RECEIPT')}
${centerText(`Order No.: ${orderNum}`)}
${centerText(getCurrentDateTime())}
${'-'.repeat(maxChars)}
`;

    receiptItems.forEach(item => {
        content += formatLine(item.item_name.substring(0, maxChars - 14), `${item.quantity} ₱${item.total_item_price}`) + '\n';
    });

    const subtotal = receiptItems.reduce((sum, item) => sum + parseFloat(item.total_item_price), 0).toFixed(2);
    const tax = (subtotal * 0.12).toFixed(2);
...
${formatLine('Amount Paid:', `₱${amountPaid}`)}
${formatLine('Change:', `₱${change}`)}
${'-'.repeat(maxChars)}
${centerText('Thank you for your purchase!')}
${centerText('Please come again')}
            </pre>
        </body>
        </html>
    `;
