...
document.getElementById('print-button').addEventListener('click', async function() {
    console.log("Print button clicked, attempting to update product stock");
    ...
    try {
        ...
        if (receiptResult.success) {
            console.log("Receipt saved, now updating stock");

            // Prepare stock updates based on receipt items
            const stockUpdates = await prepareStockUpdates(receiptData.items);
            ...
            if (stockUpdates.length > 0) {
                ...
                throw new Error("No valid stock updates to process");
            }
        } else {
            console.error('Error saving receipt:', receiptResult.error);
...
async function prepareStockUpdates(items) {
    try {
        console.log("Preparing stock updates for items:", items);
        
        // Get current stock levels for all items
        const stockResponse = await fetch('getProductStock.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(items.map(item => item.ItemID))
        });
        ...
        console.log("Received stock data:", responseData);

        // Ensure we have a valid array of stock data
        if (!responseData || !Array.isArray(responseData)) {
            throw new Error("Invalid stock data received from server");
        }

        // Convert the stock data to a Map for easier lookup
        const stockMap = new Map(
            responseData.map(item => [parseInt(item.ItemID), parseInt(item.InStock)])
        );

        // Calculate new stock levels
        const updates = items.map(item => {
            const itemId = parseInt(item.ItemID);
            const currentStock = stockMap.get(itemId);
            
            if (typeof currentStock !== 'number') {
                console.error(`No current stock found for item ${itemId}`);
                return null;
            }
            ...
            if (newStock < 0) {
                console.error(`Insufficient stock for item ${itemId}. Current: ${currentStock}, Requested: ${item.quantity}`);
                return null;
            }
        ...
        console.error("Error preparing stock updates:", error);
        throw error;
    }
}

async function updateProductStock(stockUpdates) {
    try {
        if (!Array.isArray(stockUpdates) || stockUpdates.length === 0) {
            throw new Error("No valid stock updates provided");
        }

        console.log("Sending stock updates:", stockUpdates);

        const response = await fetch('updateProductStock.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        console.log("Response from updateProductStock.php:", result);

        if (!result.success) {
            throw new Error(result.message || "Error updating product stock");
        }

        console.log("Stock successfully updated in the database");
        return result;
    } catch (error) {
...
function getReceiptData() {
    return {
        ...
        items: receiptItems.map(item => ({
            ItemID: parseInt(item.id),  // Ensure ItemID is an integer
