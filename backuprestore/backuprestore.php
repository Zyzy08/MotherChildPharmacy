...
<head>
    ...
    <title>Backup & Restore - Mother & Child Pharmacy and Medical Supplies</title>
...
<body>
    ...
    <aside id="sidebar" class="sidebar">
        ...
        <ul class="sidebar-nav" id="sidebar-nav">
            ...
            <?php if ($_SESSION['UsersPerms'] === 'on'): ?>
                <li class="nav-item">
                    <a class="nav-link" href="../backuprestore/backuprestore.php">
        ...
        <div class="modal-dialog" data-bs-backdrop="false">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title" id="modalTitle">Backup Status</h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body" id="modalBody">
                    <!-- Response message will go here -->
                </div>
            </div>
        </div>
    </div>

    <div class="modal shortermodal" id="disablebackdrop-Front2" tabindex="-1">
        <div class="modal-dialog" data-bs-backdrop="false">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 class="modal-title" id="modalTitle2">Restore Status</h3>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <script>
        ...
        function runBackup() {
            fetch('backup.php')
                ...
                .then(data => {
                    // Show the modal with the response from PHP
                    if (data.status === 'success') {
                        showModal('Success', `Backup created successfully at: <strong>${data.path}</strong>`);
                    } else {
    ...
    <script>
        document.getElementById('restoreForm').addEventListener('submit', function (event) {
            ...
            fetch('restore.php', {
                ...
                .catch(error => {
                    // Handle any errors
                    showModal2('Error', 'An error occurred: ' + error.message);
                });
        });
